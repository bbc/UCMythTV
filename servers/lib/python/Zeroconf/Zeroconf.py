...
class DNSIncoming(object):
	...
	def __repr__(self):
		try:
			return "< %s%s: id=%r, flags=%r, questions=%r, answers=%r >" % (('QUERY ' if self.isQuery() else ''),
											('RESPONSE ' if self.isResponse() else ''),
											self.id,
											self.flags,
											self.questions,
											self.answers)
...
#							traceback.print_exc()
	...
	def getReaders(self):
		result = []
		if self is not None and self.condition is not None:
			self.condition.acquire()
	...
	def handleQuery(self, msg, addr, port):
		...
		for question in msg.questions:
			if question.type == _TYPE_PTR:
				# First check if this is the special "list everything" service:
				if question.name[:22] == u'_services._dns-sd._udp':
					if out is None:
						out = DNSOutgoing(_FLAGS_QR_RESPONSE | _FLAGS_AA)
